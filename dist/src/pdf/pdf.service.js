    ...
    async generateOrderPdf(orderId, type) {
        ...
        if (order.checklists?.length > 0) {
            ly = this.ensureSpaceAndSync(page, ly, 60 + order.checklists.length * 11, pdfDoc, (v) => ly = v, (v, p) => { ry = v; if (p)
                page = p; });
            const isPhysical = order.checklists.some((c) => ['Marco', 'Tapa', 'Visor (cámara)', 'Bandeja Sim', 'Estado de pantalla'].includes(c.componentName));
            ...
            if (order.checklists.every((c) => c.notTestable)) {
                this.drawSafe(page, 'No fue posible realizar pruebas', { x: colTx('left'), y: ly, size: 7, font: boldFont, color: (0, pdf_lib_1.rgb)(0.7, 0.35, 0) });
                ly -= 12;
            }
        ...
        if (hasPartsCosts || hasLaborCosts || order.estimatedCost || order.finalCost) {
            ...
            if (hasLaborCosts)
                costData.push({ label: 'Costo de reparación', value: `$${laborCosts.toLocaleString('es-CO')}` });
